...
(function () {
	...
	CKEDITOR.plugins.add('widget', {
		...
		onLoad: function () {
			...
			CKEDITOR.addCss(
				...
				'.widget_wrapper_readonly:hover { outline:none !important; }' +
				'.widget_wrapper_readonly:hover>.cke_widget_element { outline:none !important; }' +
				'.widget_wrapper_readonly:hover .cke_widget_editable { outline:none !important; }' +
				'.widget_wrapper_readonly:hover>.cke_widget_drag_handler_container { display:none !important; }' +

				'.cke_widget_inline{' +
				'display:inline-block' +
				'}' +
				'.cke_widget_wrapper:hover>.cke_widget_element {' +
				'outline:1px solid #ffd25c;' +
				'cursor:default;' +
				'}' +
	...
	function Widget(widgetsRepo, id, element, widgetDef, startupData) {
		...
		CKEDITOR.tools.extend(this, widgetDef, {
			...
			draggable: true, //widgetDef.draggable !== false,
	...
	function blurWidget(widgetsRepo, widget) {
		...
		if (widget.isInited()) {
			...
			widgetsRepo.fire('widgetBlurred', {
				widget: widget
			});
	...
	function setupMouseObserver(widgetsRepo) {
		...
		editor.on('doubleclick', function (evt) {
			...
			return widget.fire('doubleclick', {
	...
	function setupWidgetsLifecycleStart(widgetsRepo) {
		...
		editor.on('afterInsertHtml', function (evt) {
			if (evt.data.intoRange) {
				...
				widgetsRepo.checkWidgets({
					initOnlyNew: true,
					focusInited: processedWidgetOnly
				});
		...
		return {
			...
			commit: function () {
				...
				if (focusedChanged && focused) {
					...
					widgetsRepo.fire('widgetFocused', {
						widget: focused
					});
	...
	var keystrokesNotBlockedByWidget = {
		37: 1,
		38: 1,
		39: 1,
		40: 1,
	...
	function setupEditables(widget) {
		...
		for (editableName in definedEditables) {
			...
			widget.initEditable(editableName, typeof editableDef == 'string' ? {
				selector: editableDef
			} : editableDef);
		}
	}
	...
	function writeDataToElement(widget) {
		//If image3 (using Base64Image) is base64 image, not set data-cke-widget-data="....."
		if (widget && widget.data && widget.data.src && widget.data.src.indexOf('base64') < 0) {
