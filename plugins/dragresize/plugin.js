...
(function () {
    ...
    function init(editor) {
        ...
        document.addEventListener('mouseover', function (e) {
            if (isEditable()) {
                ...
                if (e.target.tagName === 'IMG' 
                    && e.target.dataset.widget 
                    && e.target.dataset.widget === 'image'
                ) {
        ...
        function selectionChange() {
            ...
            if (selection.getType() !== CKEDITOR.SELECTION_NONE) {
                ...
                isImage = selectionEl.is('img');
                if (isImage) {
                    imageEl = selection.getStartElement().$;
                } else {
                    // for image3 pluin because image3 is wrapped by widget plugin
                    if (selectionEl.hasClass('cke_widget_wrapper')) {
                        imageEl = selectionEl.$.querySelector('img')
                        if (imageEl) {
                            isImage = true;
                        }
                    }
    ...
    Resizer.prototype = {
        ...
        updateHandles: function (box, left, top) {
            ...
            positionElement(handles.tr, box.width - 2 + left, -3 + top);
            positionElement(handles.lm, -3 + left, Math.round(box.height / 2) - 3 + top);
            positionElement(handles.rm, box.width - 2 + left, Math.round(box.height / 2) - 3 + top);
            positionElement(handles.bl, -3 + left, box.height - 2 + top);
            positionElement(handles.bm, Math.round(box.width / 2) - 3 + left, box.height - 2 + top);
            positionElement(handles.br, box.width - 2 + left, box.height - 2 + top);
        },
